...
cv2.createTrackbar("square", "setting", 0, 10000, noth)##########
while True:
    key = 0##########
    minV = cv2.getTrackbarPos("minV", "setting")
    maxV = cv2.getTrackbarPos("maxV", "setting")
    square = cv2.getTrackbarPos("square", "setting")#########
    img = cv2.imread("./img/people1.jfif")
    gray_img = cv2.cvtColor(img, cv2.COLOR_BGRA2GRAY)
    h, th = cv2.threshold(gray_img, minV, maxV, 1)
    contours, h = cv2.findContours(th,cv2.RETR_LIST,cv2.CHAIN_APPROX_SIMPLE)
    for contour in contours:
        s = cv2.contourArea(contour)##########
        if s>square:##########
            key+=1#############
            (x,y,w,h)=cv2.boundingRect(contour)
            cv2.putText(img,f"person {key}",(x,y),1,1.0,(255,0,0),1)#############
